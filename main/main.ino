...
void usbOutTransferChunk(uint32_t addr, uint32_t ep, uint32_t nbytes, uint8_t* data) {
  ...
  if (rcode) {
    if (rcode == USB_ERROR_DATATOGGLE) {
      ...
      strip.setPixelColor(0, 64, 64, 64); strip.show();
...
void readTegraDeviceID(byte *deviceID) {
  ...
  if (usb.inTransfer(tegraDeviceAddress, 0x01, &readLength, deviceID)) DEBUG_PRINTLN("Failed to get device ID!");
}
...
void setup() {
  ...
  analogWrite(13, 0); //led to indicate injection started
